...
global keep_track_of_spatial_vertex # MY CODE
keep_track_of_spatial_vertex = [] # MY CODE
global total_episodes # MY CODE
total_episodes = 0 # MY CODE
global first_time # MY CODE
first_time = True # MY CODE
# Dictionary with pair of objects interacting and their vartices ids as a list
global dict_object_vertices # MY CODE
dict_object_vertices = {} # MY CODE
# Boolean variable to include or not the objects' ids in the spatial and temporal
# nodes in the graph
global keep_objects_in_node_names # MY CODE
keep_objects_in_node_names =  False # MY CODE

# Passing over to utils.py the final edges lists of the graph.
global spatial_obj_final_edges, temp_spatial_final_edges # MY CODE
...
def get_graph(episodes, object_types={}):
    ...
    for (objs, relations, (intv_start, intv_end)) in episodes:
        ...
        if (len(combine_objects) >= 2):
            ...
            dict_key = list(reversed(combine_objects))[0] + list(reversed(combine_objects))[1]
        ...
        if (((keep_track_of_spatial_vertex == []) or (vertex_count not in keep_track_of_spatial_vertex)) and first_time ):
            if spatial_edges is 2:   
                omy = [objects[objs[0]], objects[objs[1]]]
                omy.sort()
                if keep_objects_in_node_names:
                    graph.vs()[vertex_count]['name'][qsrkey] = graph.vs()[vertex_count]['name'][qsrkey] + '_' + str(omy[0]) + '_' + str(omy[1])
                else:
                    graph.vs()[vertex_count]['name'][qsrkey] = graph.vs()[vertex_count]['name'][qsrkey]
            elif spatial_edges is 3:
                omy = [objects[objs[0]], objects[objs[1]], objects[objs[2]]]
                omy.sort()
                if keep_objects_in_node_names:
                    graph.vs()[vertex_count]['name'][qsrkey] = graph.vs()[vertex_count]['name'][qsrkey] + str(omy[0]) + '_' + str(omy[1]) + '_' + str(omy[2])
                else:
                    graph.vs()[vertex_count]['name'][qsrkey] = graph.vs()[vertex_count]['name'][qsrkey]
    ...
    for epi1, epi2 in combinations(spatial_data, 2):
        ...
        if (spatial_rel1 == spatial_rel2):
            us = graph.vs()[temporal_rel_vertex_id]['name'] + '_' + spatial_rel1
            if keep_objects_in_node_names:
                for i in so12:
                    us += str(i)
            ...
            rels12.sort()
            us = graph.vs()[temporal_rel_vertex_id]['name'] + '_' + rels12[0] + rels12[1]
            if keep_objects_in_node_names:
                for i in so12:
                    us += str(i)
            graph.vs()[temporal_rel_vertex_id]['name'] = us
